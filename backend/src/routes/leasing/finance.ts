...
leasingFinanceRouter.get(
  ...
  asyncHandler(async (req, res) => {
    res.json({ data: await leasingReadyStockSummary(req.auth!.sub) });
  }),
);
...
leasingFinanceRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await leasingReadySales({ ownerAdminId: req.auth!.sub, ...q });
...
leasingFinanceRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({ data: await daySummary({ ownerAdminId: req.auth!.sub, day }) });
  }),
);
...
leasingFinanceRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await loadSettlementsForPay(body.settlementIds, req.auth!.sub);
...
leasingFinanceRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const payment = await prisma.itgelSettlementPayment.findFirst({
      where: { id: req.params.id, ownerAdminId: req.auth!.sub },
    });
...
leasingFinanceRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const payment = await prisma.itgelSettlementPayment.findFirst({
      where: { id: req.params.id, ownerAdminId: req.auth!.sub, status: 'PENDING' },
    });
    if (!payment) throw notFound('Төлбөр олдсонгүй.');
...
leasingFinanceRouter.get(
  '/itgel/payments',
  asyncHandler(async (req, res) => {
    const rows = await listSettlementPayments({ ownerAdminId: req.auth!.sub });
