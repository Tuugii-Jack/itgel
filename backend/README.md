...
| `npm test` | Бизнес логикийн тест (34 тест) |
| `npm run typecheck` | TypeScript шалгалт |
| `npm run seed` | Өгөгдлийг цэвэрлээд шинээр үүсгэнэ |
...
**Мөнгө.** Бүх дүн бүхэл тоо (₮), float хэрэглэхгүй. `paidAmount = дүн × depositPercent / 100`
(доош тойрно), 100% үед `dueAmount = 0`. Тайлангийн ашиг зөвхөн `HANDED_OVER` захиалгаас,
`costPriceSnapshot`-оор бодогдоно.
